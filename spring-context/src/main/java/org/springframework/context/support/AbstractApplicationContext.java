...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected void publishEvent(Object event, @Nullable ResolvableType eventType) {
		...
		// 如果此时多播器还没初始化，先存储在earlyApplicationEvents
	...
	public void refresh() throws BeansException, IllegalStateException {
		...
		synchronized (this.startupShutdownMonitor) {
			...
			// 1.准备工作，设置容器状态，初始化容器标志位，设置环境变量
			prepareRefresh();

			// Tell the subclass to refresh the internal bean factory.
			// 2.获取已有的spring容器
			// 2.1 如果是xml配置文件，此时会将beanDefinition加载到容器中
			// 2.2 如果是注解配置，在调用refresh()方法前就已经加载完成(do scan的时候完成)
			ConfigurableListableBeanFactory beanFactory = obtainFreshBeanFactory();

			// Prepare the bean factory for use in this context.
			// 3.初始化bean工厂，填充属性，注册一些初始化bean
			prepareBeanFactory(beanFactory);

			try {
				// Allows post-processing of the bean factory in context subclasses.
				// 空方法，执行内部的容器后置处理增强，此时不做任何扩展
				...
				// 注册BeanPostProcessors，只是注册，并没有真正调用
				registerBeanPostProcessors(beanFactory);
				beanPostProcess.end();

				// Initialize message source for this context.
				// 国际化处理，springmvc国际化有使用
				initMessageSource();
				...
				// 多播器 - 把被观察者遍历通知观察者执行对应事件的操作委托 给多播器，来进行消息通知
				// 1. 事件源发布不同的事件
				// 2. 调用多播器的方法进行事件广播操作
				// 3. 监听器接收到具体的事件，可以验证匹配是否能处理当前事件，如果可以直接处理
				// 初始化事件监听多播器
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		// 刷新已存在的bean工厂，如果已存在，则销毁，如果不存在，创建新的DefaultListableBeanFactory
		refreshBeanFactory();
		// 返回bean工厂
		return getBeanFactory();
	}
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 为bean工厂设置默认的propertyEditor，用于对bean的属性进行设置(将string转换为对应的属性值), 在初始化的时候遍历registrar处理属性值
		beanFactory.addPropertyEditorRegistrar(new ResourceEditorRegistrar(this, getEnvironment()));

		// Configure the bean factory with context callbacks.
		// 设置添加BeanPostProcessor -- ApplicationContextAwareProcessor(用来完成某些aware对象的赋值工作)
		beanFactory.addBeanPostProcessor(new ApplicationContextAwareProcessor(this));
		// 设置忽略的aware接口，自动注入的时候忽略下面的接口
		...
		// 注册初始化Bean，如果有相同的实现(多个匹配项注入)，使用这里设置的实现
		...
		// 设置添加BeanPostProcessor -- ApplicationListenerDetector
		beanFactory.addBeanPostProcessor(new ApplicationListenerDetector(this));

		// 添加对Aspect的支持，LTW，类加载器织入
	 ...
	 * 必须在单例对象实例化之前调用，实例化之后再修改BeanDefinition无效
	...
	protected void initApplicationEventMulticaster() {
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
		// 是否有自定义的applicationEventMulticaster，将其加载到容器中
	...
	protected void registerListeners() {
		...
		// 获取容器中的ApplicationListener监听器，将其注册到多播器
		String[] listenerBeanNames = getBeanNamesForType(ApplicationListener.class, true, false);
		for (String listenerBeanName : listenerBeanNames) {
			// 讲beanName放入到多播器name集合中
			getApplicationEventMulticaster().addApplicationListenerBean(listenerBeanName);
		}

		// Publish early application events now that we finally have a multicaster...
		// 如果earlyApplicationEvents有值的情况，直接向多播器发布事件
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 如果beanFactory之前没有注册嵌入值解析器(PropertySourcesPlaceholderConfigurer)，使用默认的嵌入值解析器，解析值
		if (!beanFactory.hasEmbeddedValueResolver()) {
			beanFactory.addEmbeddedValueResolver(strVal -> getEnvironment().resolvePlaceholders(strVal));
		}

		// Initialize LoadTimeWeaverAware beans early to allow for registering their transformers early.
		// 提前注册LoadTimeWeaverAware bean
		...
		// 冻结beanDefinition定义，注册的bean定义将不被修改
